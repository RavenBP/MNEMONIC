...
#include "MNEMONIC_ProjectProjectile.h"
...
void AMNEMONIC_ProjectCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
...
void AMNEMONIC_ProjectCharacter::UpdatePlayerStats(FStatistics newStats)
{
	...
	m_pParkour->m_fTimeBetweenDashes = m_PlayerStats.m_fDashCoolDown * 1000.0f; // time in ms
	...
	/*if (ProjectileClass != nullptr)
	{
		...
		if (World != nullptr)
		{
			if (bUsingMotionControllers)
			{
				...
				World->SpawnActor<AMNEMONIC_ProjectProjectile>(ProjectileClass, SpawnLocation, SpawnRotation);
			}
			else
			{
				...
				World->SpawnActor<AMNEMONIC_ProjectProjectile>(ProjectileClass, SpawnLocation, SpawnRotation, ActorSpawnParams);
			}
		}
	}
...
void AMNEMONIC_ProjectCharacter::BeginTouch(const ETouchIndex::Type FingerIndex, const FVector Location)
{
...
void AMNEMONIC_ProjectCharacter::EndTouch(const ETouchIndex::Type FingerIndex, const FVector Location)
{
...
//void AMNEMONIC_ProjectCharacter::TouchUpdate(const ETouchIndex::Type FingerIndex, const FVector Location)
//{
...
void AMNEMONIC_ProjectCharacter::MoveForward(float Value)
{
	if (Value != 0.0f)
	{
...
void AMNEMONIC_ProjectCharacter::MoveRight(float Value)
{
	if (Value != 0.0f)
	{
		// add movement in that direction
...
void AMNEMONIC_ProjectCharacter::TurnAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerYawInput(Rate * BaseTurnRate * GetWorld()->GetDeltaSeconds());
}

void AMNEMONIC_ProjectCharacter::LookUpAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerPitchInput(Rate * BaseLookUpRate * GetWorld()->GetDeltaSeconds());
}

bool AMNEMONIC_ProjectCharacter::EnableTouchscreenMovement(class UInputComponent* PlayerInputComponent)
{
	if (FPlatformMisc::SupportsTouchInput() || GetDefault<UInputSettings>()->bUseMouseForTouch)
	{
		PlayerInputComponent->BindTouch(EInputEvent::IE_Pressed, this, &AMNEMONIC_ProjectCharacter::BeginTouch);
		PlayerInputComponent->BindTouch(EInputEvent::IE_Released, this, &AMNEMONIC_ProjectCharacter::EndTouch);

		//Commenting this out to be more consistent with FPS BP template.
		//PlayerInputComponent->BindTouch(EInputEvent::IE_Repeat, this, &AMNEMONIC_ProjectCharacter::TouchUpdate);
		return true;
	}
